...
def main(args):
    ...
    test_loader = DataLoader(CustomImageDataset(root_dir="/workspace/data/policelab_gen_dataset_light/test/", mode='test'), batch_size=args.batch, num_workers=20, shuffle=True)
...
def validate_model(
    ...
    len_per_dict = CustomImageDataset(root_dir="/workspace/data/policelab_gen_dataset_light/test/", mode='test').len_per_dir()
    ...
    confidence_scores_fake = []
    confidence_scores_real = []
    with torch.no_grad():
        for i, (img, label, dir_name) in enumerate(tqdm.tqdm(val_loader)):
            ...
            confidence_scores_fake.extend(probs[label == 1].squeeze().cpu().detach().numpy())
            confidence_scores_real.extend(probs[label == 0].squeeze().cpu().detach().numpy())
    ...
    plt.hist(confidence_scores_fake, bins=bins, alpha=0.5)
    plt.xlabel('Confidence Score')
    plt.ylabel('Frequency')
    plt.title('Confidence Score Distribution per Epoch')
    plt.savefig(os.path.join(os.path.join(os.path.join(model_save_dir,'confidence_graph'),'fake'),'{}_epoch.png'.format(args.ckpt_used)))
    plt.cla()   # clear the current axes
    plt.clf()   # clear the current figure
    plt.close() # closes the current figure

    plt.hist(confidence_scores_real, bins=bins, alpha=0.5)
    plt.xlabel('Confidence Score')
    plt.ylabel('Frequency')
    plt.title('Confidence Score Distribution per Epoch')
    plt.savefig(os.path.join(os.path.join(os.path.join(model_save_dir,'confidence_graph'),'real'),'{}_epoch.png'.format(args.ckpt_used)))
    plt.cla()   # clear the current axes
    plt.clf()   # clear the current figure
    plt.close() # closes the current figure
