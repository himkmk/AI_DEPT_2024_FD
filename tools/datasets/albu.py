...
# 20240325
# from tools.datasets.custom_augmentation import WebpCompression
from .custom_augmentation import WebpCompression

class WebpAugmentation(ImageOnlyTransform):    
    def __init__(
        self,
        always_apply=False,
        p=1,
        ...
        super(WebpAugmentation, self).__init__(always_apply, p)
        self.p = p
    ...
    def apply(self, img, **params):
        ...
        quality = np.random.randint(low=min(self.qf_range), high=max(self.qf_range))
        return WebpCompression(img, quality=quality)